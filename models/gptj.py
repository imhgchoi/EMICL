...
class GPTJWrapper(object):
    ...
    def generate(self, args, query, return_logits=True, return_yes_no_prob=False, verbose=False):
        ...
        response = response.replace('Ċ','').replace('Ġ',' ').strip()
        
        if verbose :
            print('query:', query)
            print('response:', response)
        
        if return_logits or return_yes_no_prob:
            map_dict = {
                'yes':[8505,3763], 'yes.':[8505,3763], 'Yes':[3363,5297], 'Yes.':[3363,5297],
